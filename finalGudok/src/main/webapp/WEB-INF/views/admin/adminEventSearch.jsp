      ...
      <div class="container box">
		...
		<div class="input-group" >
			...
			<form action="adminEventSearch.do" method="post" enctype="multipart/form-data">
              ...
              <select class="custom-select" id="post_yn" name="post_yn" style="margin-left:-4px; width:150px">
                <option value="All"<c:if test="${post_yn eq 'All' }">selected='selected'</c:if> >모두</option>
                <option value="Yes"<c:if test="${post_yn eq 'Yes' }">selected='selected'</c:if> >게시</option>
                <option value="No"<c:if test="${post_yn eq 'No' }">selected='selected'</c:if> >중지</option>      
              </select>
            ...
            <br>
                ...
                <input type="hidden" name="post_yn" value="${post_yn }">
        ...
        <thead>
          <tr>
            ...
            <th onclick='event.cancelBubble=true' style="width:15%">조회수</th>
            <th onclick='event.cancelBubble=true' style="width:15%">게시여부</th>
          </tr>
        </thead>
        <tbody >
	        <c:choose>
		      			<c:when test="${fn:length(list1)>0 }">
					        <c:forEach var="b" items="${list1 }" varStatus="cnt">
					          <tr>
					          		<td onclick='event.cancelBubble=true'><input type="checkbox" class="common" id="eventNo${cnt.index}" name="eventNo" value="${b.bBoard_no }"></td>
                       ...
                       <!-- 이전 -->
                        <c:if test="${pi.currentPage eq 1 }">
                   		...
                   		<c:if test="${pi.currentPage gt 1 }">
                     		<c:url var="blistBack" value="adminEventSearch.do">
                        		<c:param name="page" value="${pi.currentPage-1 }"/>
                        		<c:param name="post_yn" value="${post_yn }"/>
                  ...
                  <c:forEach var="p" begin="${pi.startPage }" end="${pi.endPage }">
                     ...
                     <c:if test="${p eq pi.currentPage }">
                           ...
                           <c:if test="${p ne pi.currentPage }">
                              <c:url var="blistCheck" value="adminEventSearch.do">
                        ...
                        <c:if test="${pi.currentPage eq pi.maxPage }">
                   		...
                   		<c:if test="${pi.currentPage lt pi.maxPage }">
                     		<c:url var="blistAfter" value="adminEventSearch.do">
                        		<c:param name="page" value="${pi.currentPage+1 }"/>
                        		<c:param name="post_yn" value="${post_yn }"/>
                        		<c:param name="searchType" value="${searchType }"/>
                        		<c:param name="keyword" value="${keyword }"/>
      ...
      function getBannerList(){
		 ...
		 var searchType='${searchType}';
     	 ...
     	 $.ajax({
     	 	...
     	 	url:"eventSearchListChange.do",
     	 	data:{"page":page, "searchType":searchType, "keyword":keyword, "post_yn":post_yn},
     	 	dataType:"json",
     	 	success:function(data){
     	 		...
     	 		var $th;
     	 				...
     	 				for(var i in data.adminEventSearch){
     	 					
     	 				$tr=$("<tr id='cursor'>");
     	 				$td=$("<td onclick='event.cancelBubble=true'>");
     	 				$checkBox=$("<input type='checkbox' class='common' name='eventNo'>").val(data.adminEventSearch[i].bBoard_no);     	 			
     	 				$bBoard_no=$("<td onclick='event.cancelBubble=true'>").text(data.adminEventSearch[i].bBoard_no).hide();
     	 				$rownum=$("<td onclick='event.cancelBubble=true'>").text(data.adminEventSearch[i].rownum);
     	 				$bTitle=$("<td>").text(data.adminEventSearch[i].bTitle);
     	 				$bRead_num=$("<td onclick='event.cancelBubble=true'>").text(data.adminEventSearch[i].bRead_num);
     	 				$eE_status=$("<td onclick='event.cancelBubble=true'>").text(data.adminEventSearch[i].eE_status);
     	 				if($eE_status.text()=="N"){
     	 					$eE_status.text("중지");
  ...
  // 선택 삭제
	  ...
	  function eDelete(){
     				...
     				if(sendCnt == 0){
	     				...
	     				location.href="eventSearchListDelete.do?sendArr="+sendArr+"&page="+page+"&searchType="+searchType+"&keyword="+keyword+"&post_yn="+post_yn;
	     			}
   				}
